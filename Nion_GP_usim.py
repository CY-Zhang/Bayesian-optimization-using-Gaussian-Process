...
for _ in range(nrep):
  ...
  # start_point = [[0.5241422520927759, 0.5498038268673789, 0.4528721606539906, 0.46746559415164934, 0.5269649425176939]]
  # start_point = [[0.49887509, 0.49656584, 0.49973694, 0.50943868, 0.5108666 ]]
  start_point = [[0.4918346 , 0.48972612, 0.50277167, 0.51270355, 0.50741157]]
  print(start_point)
  ...
  idx = 0
  filename = 'GPrun_' + str(Niter) + 'iter_' + str(idx) + '_abr_coeff_UCB_default_refine.npy'
  ...
  filename = 'GPrun_' + str(Niter) + 'iter_' + str(idx) +'_prediction_UCB_default_refine.npy'
  print(path + filename)
  np.save(path + filename, np.array(obj_list))

  filename = 'GPrun_' + str(Niter) + 'iter_' + str(idx) +'_ronchigram_UCB_default_refine.npy'
  print(path + filename)
